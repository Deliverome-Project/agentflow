...
from .theme import BERRY, CORAL, setup_plots
...
def draw_population(ax, prepared, gate, parent_mask):
    ...
    density = LinearSegmentedColormap.from_list("deliverome_density", ["#e8f3f1", "#7aab9f", "#244840"])
    data = prepared.transformed.loc[parent_mask, gate["channels"]]
    if gate["kind"] == "range":
        channel = gate["channels"][0]
        if len(data):
            ax.hist(data[channel], bins=100, color=BERRY, alpha=0.8)
        ax.set_xlabel(channel)
        ax.set_ylabel("Events")
    else:
        x, y = gate["channels"]
        if len(data):
            ax.hexbin(data[x], data[y], gridsize=80, mincnt=1, bins="log", cmap=density)
        ax.set_xlabel(x)
        ax.set_ylabel(y)
    if not len(data):
    ...
    if gate["kind"] == "range":
        for bound in gate["bounds"]:
            if bound is not None:
                ax.axvline(bound, color=CORAL, lw=1.5)
    elif gate["kind"] == "polygon":
        ax.add_patch(Polygon(gate["vertices"], fill=False, edgecolor=CORAL, linewidth=1.5))
    else:
        x0, x1, y0, y1 = gate["bounds"]
        ax.add_patch(Rectangle((x0, y0), x1 - x0, y1 - y0, fill=False, edgecolor=CORAL, linewidth=1.5))
    ax.autoscale_view()
